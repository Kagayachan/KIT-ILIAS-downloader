...
static DEFAULT_DESKTOP_URL: &str = "https://ilias.studium.kit.edu/ilias.php?baseClass=ilDashboardGUI&cmd=show";
static DEFAULT_ALL_COURSES_URL: &str =
	"https://ilias.studium.kit.edu/ilias.php?cmdClass=ilmembershipoverviewgui&baseClass=ilmembershipoverviewgui";
	...
	if opt.keep_session {
			...
			.context("failed to load previous session")
		{
			Ok(ilias) => {
				...
				if let Err(e) = ilias.get_html(DEFAULT_ALL_COURSES_URL).await {
					error!(e);
				} else {
...
async fn real_main(mut opt: Opt) -> Result<()> {
	...
	let sync_url = if let Some(url) = &ilias.opt.sync_url {
		url.clone()
	} else if ilias.opt.desktop {
		DEFAULT_DESKTOP_URL.to_owned()
	} else {
		// default: all courses on the personal membership overview
		DEFAULT_ALL_COURSES_URL.to_owned()
	};
