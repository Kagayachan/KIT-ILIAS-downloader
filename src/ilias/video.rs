...
static XOCT_STREAMS_REGEX: Lazy<Regex> =
	Lazy::new(|| Regex::new(r#"(\{"streams"[\s\S]+?),\s*\{"paella_config_file"#).unwrap());

fn parse_opencast_json(html: &str) -> Result<serde_json::Value> {
	if let Some(cap) = XOCT_STREAMS_REGEX.captures(html) {
		let json = cap.get(1).context("xoct streams json not found")?.as_str();
		return serde_json::from_str(json.trim()).context("invalid xoct streams json");
	}
	...
	serde_json::from_str(json.trim()).context("invalid xoct player json")
}
...
pub async fn download(path: &Path, relative_path: &Path, ilias: Arc<ILIAS>, url: &URL) -> Result<()> {
	...
	log!(2, "{}", html);
	let json = parse_opencast_json(&html)?;
	log!(2, "{}", json);
	let streams = json
		.get("streams")
